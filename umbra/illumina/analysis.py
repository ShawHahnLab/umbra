...
class AnalysisClassic(Analysis):
    ...
    def __sample_paths_for_num(self, sample_num, strict=True):
        fps = []
    ...
    def sample_paths(self, strict=True):
        ...
        """
        sample_paths = {}
        sample_numbers = [i+1 for i in range(len(self.sample_sheet["Data"]))]
        sample_names = [row.get("Sample_Name") for row in self.sample_sheet["Data"]]
        for s_num, s_name in zip(sample_numbers, sample_names):
            sps = self.__sample_paths_for_num(s_num, strict)
